...
class CaptchaController extends Controller
{
    public function image(Request $request)
    {
     ...
     public function mathCaptcha(Request $request)
    {
        $num1 = mt_rand(1, 12);
        $num2 = mt_rand(1, 12);
    ...
    public function verifyMath(Request $request)
    {
        $submitted = (int) $request->input('captcha_answer');
        $correct   = (int) session('math_captcha_answer');

        $isValid = session()->has('math_captcha_answer') && $submitted === $correct;
